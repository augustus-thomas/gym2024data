#!/home/awth/miniconda3/envs/forec/bin/python3
# coding: utf-8
...
# read and parse the R input
def read_R_file(file_name):
    # get filename
    r_output_file = file_name
    with open(r_output_file, 'r') as file_obj:
        raw = file_obj.read()
    # parse html
    ...
    for i, l in enumerate(clean_forecasts):
        for j, val in enumerate(l):
            clean_forecasts[i][j] = float(val)
    ...
    for entry in raw_string_name_app:
        LastName = re.findall('LastName == "(.+?)"', entry)
        Apparatus = re.findall('Apparatus == "(.+?)"', entry)
        clean_name.append(LastName[0])
        clean_app.append(Apparatus)
    print(clean_app)
    print(len(clean_app))
    print(clean_name)
    print(len(clean_name))
    clean_forecasts = dict(zip(clean_name, clean_app))
    print(clean_forecasts)
    #print(clean_name)
    #print(clean_app)
    exit()

read_R_file(file_name="./R/USOPC-time-series-forecasting.html")
...
# assert type(forecasts) == type({}), type(forecast)
